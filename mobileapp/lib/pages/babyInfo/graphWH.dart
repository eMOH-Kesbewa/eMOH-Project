...
class GraphWH extends StatefulWidget {
  final Widget child;

  GraphWH({Key key, this.child}) : super(key: key);

  _GraphWHState createState() => _GraphWHState();
}

class _GraphWHState extends State<GraphWH> {
  ...
  _asyncMethod() async {
    fetchWeightHeight().then((it) {
      for (var i = 0; i < it.length; i++) {
        ...
        double wval = it["${hp}"].toDouble();
        double hval = it["${wp}"].toDouble();

        var x = i / 5;
        month = x.round();

        plot.add(Points(hval, wval));
      }
